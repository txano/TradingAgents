...
- **Beat streak fading**: when a company’s recent EPS surprises show a clear shrinking trend (e.g., 30% → 15% → 5%), consensus estimates are catching up and the streak is fragile. A beat may still occur but the magnitude is likely smaller and may be interpreted as a disappointment — downgrade beat_score by 1–2 and raise the probability of a miss.
...
- **Guidance caution for high‑multiple growth names**: when the stock trades at a significant premium (forward P/E >30, or P/S >10) and faces any macro, competitive, or decelerating‑growth headwind, assume the market will punish anything short of a significant guidance raise. Bias guidance_score toward Bearish/Neutral in these cases — a reaffirmation alone can tank the stock.
- **Management pivot risk**: if a new CEO has recently been appointed or a strategic transformation is underway, consider that a fresh, optimistic tone is possible and may surprise the crowd; do not default to “cautious” without evaluating the new leader’s incentives.
...
- **Overbought run-up filter**: if the stock has risen more than 15% in the past month, or more than 5% in the past 5 trading days, downgrade setup_score by 1–2 points and reduce pre-earnings position confidence by one notch. Even a clean beat is likely to trigger profit-taking.
  - **Hard SKIP trigger**: if the stock has rallied >20% in the past 3 months and is sitting within 3% of its 52-week high, the risk of a "sell the news" event is extremely high regardless of fundamental quality — set the signal to SKIP and note the overbought condition as the primary reason.
- **Revenue and earnings quality**: always scrutinise revenue growth and margin trends alongside EPS estimates. A beat driven solely by cost-cutting or one-time items with declining revenue is a bearish signal; a beat with accelerating revenue and expanding margins is truly bullish. A revenue miss can sink a stock even on a big EPS beat, especially for subscription-transition businesses.
