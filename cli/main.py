# Load .env before any tradingagents import so API keys and TRADINGAGENTS_*
# config overrides are in os.environ when DEFAULT_CONFIG is first built.
...
def main_menu(ctx: typer.Context) -> None:
    ...
    choices = [
        ...
        questionary.Choice("  Learn       reflect on all trades → auto-improve",   value="learn"),
    ...
    dispatch = {
        ...
        "learn":        lambda: learn(provider=None, model=None, apply_proposal_path=None, force=False, dry_run=False, skip_reflect=False, no_weights=False, no_prompt_edits=False),
        "improve":      lambda: improve(),
        "correlation":  lambda: correlation(),
        "stats":        lambda: stats(),
        "weights":   lambda: allocation_weights(beat=None, guidance=None, setup=None, fundamentals=None, reset=False),
...
# ── Extracted command registrations ──────────────────────────────────────────
# All commands below are defined in cli/commands/ and registered here so that
# main.py stays close to the upstream TauricResearch structure.

from cli.commands.calendar  import earnings_calendar
from cli.commands.screen    import screen, run_screening
from cli.commands.reflect   import reflect
from cli.commands.improve   import improve
